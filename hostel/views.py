...
@login_required
def authority_in_out_status(request):
    authority = get_object_or_404(Authority, user=request.user)
    
    # Process all students
    all_students = Student.objects.select_related('room__hostel').all()
    ...
    for student in all_students:
        ...
        # Logic: If last record is OUT, they are outside. Otherwise (IN or No Record), they are inside.
        if last_log and last_log.event_type == 'OUT':
            student.last_out_time = last_log.timestamp
            student.out_purpose = last_log.purpose
            outside_students.append(student)
        else:
            if last_log and last_log.event_type == 'IN':
                 student.last_in_time = last_log.timestamp
            inside_students.append(student)
    ...
    return render(request, "authority_in_out.html", context)
